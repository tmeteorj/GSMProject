...
public class EventEdge {
	...
	public static class EventMapper extends MapReduceBase implements
		...
		public void map(LongWritable key, Text value,
			...
			if (info[3].compareTo(info[4]) <= 0) {
				m_key.set(info[3] + "," + info[4] + ","
						+ info[1].substring(0, 8));
				flag = ",0";
			} else {
				m_key.set(info[4] + "," + info[3] + ","
						+ info[1].substring(0, 8));
				flag = ",1";
			}
	...
	public static class EventReducer extends MapReduceBase implements
	...
	public static void main(String[] args) throws Exception {
		...
		JobConf conf = new JobConf(EventEdge.class);
		conf.setJobName("EventEdge"); // 设置一个用户定义的job名称
		conf.setOutputKeyClass(Text.class); // 为job的输出数据设置Key类
		conf.setOutputValueClass(Text.class); // 为job输出设置value类
		conf.setMapperClass(EventMapper.class); // 建边
		conf.setReducerClass(EventReducer.class); // 为job设置Reduce类
